...
@Stateless
@LocalBean
public class ServiceFilRouge implements ServiceFilRougeRemote, ServiceFilRougeLocal {
...
//		} else {
//			System.out.format("%d %s %s%n", profil.getPrfId(), profil.getPrfNom(), profil.getPrfPrenom());
//		}
		...
		List<Profil> result = (List<Profil>) query.getResultList();
       	for (Profil profil: result) {
       		System.out.format("%d %s %s (%d %s)",
       				profil.getPrfId(), profil.getPrfNom(), profil.getPrfPrenom()
       				, profil.getDroit().getDroitId(), profil.getDroit().getDroitIntitule()
          		);
       		if (profil.getPromotion() == null) {
       			...
       			System.out.format(" (%d %s %s)%n",
       					profil.getPromotion().getPrmId(), profil.getPromotion().getPrmIntitule(), profil.getPromotion().getPrmLieu()
       				);
       		}
	...
	private void parcourtRecursif(List<Competence> enfants) {
		if (enfants == null) return; // arret du parcourt recursif
	...
	public List<Competence> skillsTree(Boolean validated) {
    	Query query = entityManager.createQuery("SELECT c FROM Competence c WHERE c.competence IS Null");
		@SuppressWarnings("unchecked")
...
//						, competence.getCompetence() == null ? "null" : competence.getCompetence().getCompIntitule()
//						, competence.getCompetences() == null ? "null" : competence.getCompetences().get(0).getCompIntitule()
//						);
//			}
//		}
		return result;
	...
	public List<Promotion> lookForClass(Promotion promotion) {
		...
		if (promotion.getPrmId() != null) {
			condition = concatConditionString(condition, "p.prmId=" + String.valueOf(promotion.getPrmId()));
		}
		if (promotion.getPrmIntitule() != null) {
			condition = concatConditionString(condition, "p.prmIntitule like '%".concat(promotion.getPrmIntitule()).concat("%'"));
		}
		if (promotion.getPrmDatedebut() != null) {
			condition = concatConditionString(condition, "p.prmDatedebut='".concat(simpleDateFormat.format(promotion.getPrmDatedebut()).concat("'")));
// ci-dessous on passe d'une date java.util a une date java.sql ; ce qui n'est pas utile pour nous
// nous nous traduisons une Date en  String 
//			statement.setDate(compteurCondition++, new Date(personne.getDateNaiss().getTime()));
		}
		if (promotion.getPrmDatefin() != null) {
			condition = concatConditionString(condition, "p.prmDatefin='".concat(simpleDateFormat.format(promotion.getPrmDatefin()).concat("'")));
		}
		if (promotion.getPrmLieu() != null) {
			condition = concatConditionString(condition, "p.prmLieu like '%".concat(promotion.getPrmLieu()).concat("%'"));
		}
	...
	private String concatConditionString(String buildString, String condition) {
		buildString = buildString.concat(buildString.length() > 0 ? " and " : " where ").concat(condition);
		return buildString;
	}
